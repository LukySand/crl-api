...
async function seedFamilies() {
    ...
    for (const family of families) {
        ...
        await prisma.family.upsert({ where: { id }, update: rest, create: { id, ...rest } });
    }
...
async function seedSchedules(placeIds: Map<string, number>, feeIds: Map<string, number>) {
    ...
    const byKey = new Map<string, { id: number; fee_id: number; day_of_week: number }>();
    ...
    for (const [placeName, feeName, day, from, to] of schedules) {
        ...
        const row = await prisma.schedule.upsert({
            where: {
                place_id_day_of_week_start_time: { place_id, day_of_week: day, start_time },
            },
            update: { fee_id, end_time: toTime(to) },
            create: { place_id, fee_id, day_of_week: day, start_time, end_time: toTime(to) },
        });
...
async function seedBookings(
    ...
    const bookings: [
        ...
        [BOOKING.pendienteF5, "Cancha de fútbol 5 «A»|6|10:00", USER.socioRodrigo, 1, "Pendiente", null],
        [
    ...
    for (const [id, key, user_id, weeks, status, notes] of bookings) {
        ...
        try {
            ...
            if (error?.code === "P2002") {
                console.warn(`El turno ${key} ya está reservado, se saltea la reserva de ejemplo.`);
                continue;
            }
            throw error;
        }
...
async function main() {
    ...
    if (esProduccion && process.env.SEED_DEMO !== "true") {
        console.log("Producción: se siembran solo los roles. Usá SEED_DEMO=true para forzar el resto.");
        return;
    }
